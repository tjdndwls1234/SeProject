...
서울시립대학교 컴퓨터과학부 2024년 소프트웨어공학 프로젝트로, Software Development Life-Cycle 을 기반으로 객체지향 소프트웨어공학 방법론을 적용하여 **수강신청 도우미** 개발
...
- **String Boot**: 웹 애플리케이션의 기본 프레임워크로 사용된다. RESTful API를 신속하게 개발할 수 있도록 돕는다.
- **MyBatis**: SQL쿼리를 매핑하여 데이터베이스와의 상호작용을 간편하게 처리한다.
- **Thymeleaf**: 서버 사이드 템플릿 엔진으로, HTML 파일을 동적으로 생성하는 데 사용된다.
- **Lombok**: Getter, Setter 등을 자동으로 생성하여 코드의 가독성을 높인다다.
- **Servlet**: HTTP 요청을 처리하고, 세션 관리를 위해 사용된다.
- **JUnit**: 단위 테스트를 위해 사용된다.
...
- **Service**
  ...
  - `TimetableService.java`: 시간표를 관리하는는 비즈니스 로직을 처리한다.
...
- **Domain**: Domain 파일은 데이터베이스 테이블과 매핑되는 주요 비즈니스 엔티티를 정의하며, 각 클래스는 속성과 관련된 비즈니스 로직을 포함한다.

- **DTO(Data Transfer Object)**: DTO 파일은 클라이언트와 서버 간의 데이터 전송을 위한 객체로, 필요한 데이터만 포함하여 API 요청 및 응답 시 사용된다.
...
백엔드 코드는 JUnit을 사용해 테스트했고, 프론트엔드 코드는 백엔드와 통합된 후 기능 테스트를 진행하며 테스트되었다.

- **회원가입**: T1에 해당하는 테스트 케이스로 정상 작동을 확인했다.
- **로그인**: T2에 해당하는 테스트 케이스로 정상 작동을 확인했다.
- **강의 조회**: T7에 해당하는 테스트 케이스로 정상 작동을 확인했다.
- **강의 조회 필터링**: T5, T6에 해당하는 테스트 케이스로 정상 작동을 확인했다.
- **선수/후수 과목 조회**: T7에 해당하는 테스트 케이스로 정상 작동을 확인했다.
- **시간표**: T3에 해당하는 테스트 케이스로 백엔드에서 정상 작동을 확인했지만 프론트엔드에서 구현 난이도로 인해 출력하는 기능을 개발하지 못했다.
- **성적표**: T8에 해당하는 테스트 케이스로 백엔드에서 정상 작동을 확인하고, 프론트엔드에서 더미데이터로 동작을 확인했지만 통합하는 과정에서 문제가 있어 기능 개발이 완료되지 못했다.
